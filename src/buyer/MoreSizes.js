...
import { IonicIcon,Colors, Styles} from '../bootstrap'
    ...
    return (
      <View style={Styles.container}>
        ...
        <View style={styles.menu}>
            ...
            <Text style={[styles.menu_item,{fontWeight:'500'}]}>WANT</Text>
        </View>
        ...
        <ScrollView contentContainerStyle={{flexGrow:1}} showsVerticalScrollIndicator={false}>
            {
                this.shoeSizes().map((size,index)=>(
                    <View key={index} style={styles.size_item}>
                        ...
                        <View>
                        <ToggleSwitch isOn={index==0? true: false} offColor={'#f2f2f2'} onColor={'#000'}  onToggle={(isOn)=>{}} />
                        </View>
                    </View>
                ))
            }
        ...
        <View style={styles.footer}>
            <TouchableOpacity onPress={()=>goBack()}>
            <Text style={{color:Colors.white,fontSize:15}}>SAVE WANT</Text>
            </TouchableOpacity>
        </View>
      </View>
    );
